          ...
          <td>
            <div class="inline">
              <template v-if="props.row.blog_post_slug">
                ...
                <button type="button" class="btn btn-sm bg-navy btn-pin"
                        :class="[props.row.blog_post_is_sticky?'pinned':null]"
                        :title="props.row.blog_post_is_sticky?$t('blog.info_sticky'):$t('blog.info_not_sticky')"
                        @click="togglePin(props.row)"><i class="fa fa-thumb-tack"></i>
                </button>
              </template>
            </div>
          </td>
...
<script>
  ...
  export default {
    ...
    methods: {
      togglePin (row) {
        row.blog_post_is_sticky = !row.blog_post_is_sticky
        axios.post(`/ajax/admin/blog/post/${row.blog_post_is_sticky?'pin':'unpin'}/${row.blog_post_slug}`)
